...
class SetupViewModel @Inject constructor(
            ...
            signInUseCase(activity)
                .onSuccess { pendingIntent ->
                    ...
                    if (pendingIntent != null) {
                        ...
                        // GIS returned hasResolution=false, AuthRepositoryImpl already
                        // resolved the email from the access token and set the credential
    ...
    fun onAuthResult(intent: android.content.Intent?, activity: Activity) {
        viewModelScope.launch {
            if (intent == null) {
                _uiState.update { it.copy(error = "Sign in cancelled") }
                return@launch
            }
            // Call signIn again -- GIS will now return hasResolution=false with accessToken,
            // and AuthRepositoryImpl will resolve the email from the token
            ...
            signInUseCase(activity)
                ...
                .onFailure { e ->
                    Log.e("[SetupViewModel]", "Re-auth failed", e)
